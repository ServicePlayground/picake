...
Picake 프로젝트를 AWS EC2로 배포하는 가이드입니다. 비용 절감을 위해 AWS App Runner 대신 EC2를 사용하며, RDS 대신 EC2에 PostgreSQL을 직접 설치하여 데이터베이스 비용을 절감합니다.
...
2. **이름**: `picake-backend-staging`
3. **AMI**: Amazon Linux 2023 kernel 6.1 (Arm)
4. **인스턴스 타입**: t4g.small(staging용)
5. **키 페어**: 새 키 페어 생성
   - 키 페어 이름: `picake-backend-staging-key`
   - 키 페어 타입: RSA
   - 프라이빗 키 파일 형식: `.pem`
...
6. 연결 설정:
   - **인스턴스**: `picake-backend-staging` 선택
   - **프라이빗 IP 주소**: 자동 선택
7. **연결** 클릭
...
```bash
AWS 콘솔에서 확인
# EC2 → 인스턴스 → picake-backend-staging → 퍼블릭 IPv4 주소
```
...
chmod 400 picake-backend-staging-key.pem

# SSH 접속 (SSH 클라이언트에서 예시코드 확인)
ssh -i picake-backend-staging-key.pem ec2-user@<EC2_PUBLIC_IP>
```
...
# Yarn 4 (Corepack) — /usr/bin 심링크 생성에 sudo 필요
...
CREATE DATABASE picake_staging_db;
# .env.staging의 DATABASE_URL에서 사용하는 사용자 이름, 비밀번호 확인(비밀번호는 Notion계정에 저장)
CREATE USER picake_admin WITH PASSWORD 'your_password';
-- 데이터베이스 생성 권한 부여 (Prisma 마이그레이션을 위한 shadow database 생성에 필요)
ALTER USER picake_admin CREATEDB;
-- 권한 부여
GRANT ALL PRIVILEGES ON DATABASE picake_staging_db TO picake_admin;
-- 데이터베이스 소유자 변경
ALTER DATABASE picake_staging_db OWNER TO picake_admin;
...
#### 3.3.2 pg_hba.conf 파일 내용 확인

```bash
# pg_hba.conf 파일 내용 확인
sudo cat /var/lib/pgsql/data/pg_hba.conf
```

#### 3.3.3 pg_hba.conf 파일 수정
...
psql -h localhost -p 5432 -U picake_admin picake_staging_db
```
...
- **필수 Secrets** (GitHub → Settings → Secrets and variables → Actions):
  - `STAGING_EC2_HOST` — Elastic IP 또는 EC2 퍼블릭 IPv4
  - `STAGING_EC2_SSH_KEY` — `picake-backend-staging-key.pem` 파일 전체 내용
  - `STAGING_DATABASE_PASSWORD` — `picake_admin` DB 비밀번호
- 배포 시 워크플로가 추가로 참조하는 Secrets: `STAGING_JWT_SECRET`, OAuth, S3 등 (`.github/workflows/deploy-staging-backend-ec2.yml` 참고)
- EC2 배포 디렉터리: `~/picake` / PM2 프로세스명: `picake-backend-staging`
...
```bash
sudo nano /etc/nginx/conf.d/picake-backend-staging.conf
```
...
server {
    listen 80;
    server_name api-staging.picakes.com; # 또는 EC2 퍼블릭 IP

    # 로그 설정
    access_log /var/log/nginx/picake-backend-staging-access.log;
    error_log /var/log/nginx/picake-backend-staging-error.log;
...
EC2 인스턴스를 도메인(`api-staging.picakes.com`)에 연결합니다.
...
2. `picakes.com` 호스팅 영역 선택

### 6.3 A 레코드 생성

1. Route53 → 호스팅 영역 → `picakes.com` → **레코드 생성** 클릭
2. 레코드 설정:
...
nslookup api-staging.picakes.com
```

### 6.5 도메인 연결 확인

```bash
# 브라우저에서 접속 (여기서 안된다면 깃허브 워크플로우 실행 후 > 다시 확인, 그래도 안된다면 SSL인증서 발급후 > 워크플로우 실행후 > 다시 확인)
http://api-staging.picakes.com/health
```
...
```bash
sudo certbot --nginx -d api-staging.picakes.com
```
...
```bash
# HTTPS로 접속 확인
curl https://api-staging.picakes.com/health

# 또는 브라우저에서 접속
https://api-staging.picakes.com/health
```
...
- 버킷 유형: 범용
- 버킷 이름: picake-uploads-staging-apne2
- 객체 소유권: ACL 비활성화됨
- 모든 퍼블릭 액세스 차단
- 버전 관리 OFF
- 기본 암호화 유형: SSE-S3
...
- 2단계
  ...
  - Origin: browse S3 버튼 클릭 > 해당 버킷 선택(picake-uploads-staing-apne2)
  - Origin path: (생략)
...
2. 정책 복사 및 저장
   ...
   - 2-2. AWS > S3 > picake-uploads-staging-apne2 버킷 클릭 > 권한 탭 > 버킷 정책 편집 > CloudFront에서 복사한 정책을 JSON 편집기에 붙여넣기 > 저장
...
4. (DNS 레코드 생성) AWS > Route53 > 호스팅 영역 > picakes.com > 레코드 생성 > Type: CNAME, 위 3번에서 발급된 CNAME이름,CNAME값 입력 > 생성
...
graph TB
    subgraph "클라이언트"
        User[사용자/프론트엔드<br/>staging.picakes.com 등]
        GitHub[GitHub Repository]
    end

    subgraph "AWS Route53"
        Route53[Route53 DNS<br/>picakes.com]
    end
    ...
    subgraph "AWS EC2 인스턴스"
        ...
        subgraph "백엔드 애플리케이션"
            Backend[NestJS Backend<br/>포트: 8080<br/>PM2: picake-backend-staging]
        end

        subgraph "데이터베이스"
            PostgreSQL[PostgreSQL 16<br/>포트: 5432<br/>picake_staging_db]
        end
    end

    subgraph "AWS S3"
        S3[S3 버킷<br/>picake-uploads-staging-apne2<br/>정적 파일 저장]
    end

    subgraph "AWS CloudFront"
        CloudFront[CloudFront CDN<br/>picake-static-staging<br/>static-staging.picakes.com<br/>SSL: ACM 인증서]
    end
    ...
    subgraph "SSL 인증서"
        LetsEncrypt[Let's Encrypt<br/>api-staging.picakes.com]
        ACM[ACM Certificate<br/>static-staging.picakes.com]
    end
