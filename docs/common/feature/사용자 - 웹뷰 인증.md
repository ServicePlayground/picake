...
- **소셜 로그인**: userId/password 외에 Google/Kakao/Apple OAuth를 web-user(`apps/web-user/src/app/auth/login/*`, `register/*`)에서 code flow로 직접 처리 (브릿지 없이 웹뷰에서 진행). Apple은 web-user(소비자)만 대상이며, 자세한 provider 추가/수정 패턴은 `social-login` skill 참고.
- **앱스토어 심사용 로그인**: `POST /v1/consumer/auth/review-login` — 심사관 계정으로 로그인해야 하는 앱스토어 심사 요구사항에 대응하기 위한 별도 플로우 (마이페이지 버전 정보 화면의 `ReviewLoginBottomSheet`에서 진입)
