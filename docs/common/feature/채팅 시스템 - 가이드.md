...
Picake의 채팅 시스템은 사용자와 판매자 간의 실시간 메시지 교환을 지원합니다. 메시지 전송은 WebSocket(Socket.IO)을 통해 처리하고, 메시지 조회 및 채팅방 관리는 REST API를 통해 처리하는 하이브리드 아키텍처를 통해 안정성과 실시간성을 모두 확보했습니다.
...
**ConsumerChatController** (`apps/backend/src/apis/consumer/controllers/chat.controller.ts`)

- 사용자용 채팅 API
- 엔드포인트: `/v1/consumer/chat-room/*`
- 인증: consumer JWT만 허용
- 제공 기능:
  - 채팅방 생성/조회
  - 채팅방 목록 조회
...
- `/chat` - 채팅방 목록 페이지 (현재 `next.config.ts`에서 `/`로 리다이렉트)
...
사용자 → [스토어 상세 페이지에서 채팅 시작 버튼 클릭]
    ↓
프론트엔드 → POST /v1/consumer/chat-room { storeId }
    ↓
...
채팅방 입장
    ↓
프론트엔드 → GET /v1/consumer/chat-room/:roomId/messages?page=1&limit=50
    ↓
...
채팅방 입장 시
    ↓
프론트엔드 → POST /v1/consumer/chat-room/:roomId/read
    ↓
...
```
프론트엔드 → GET /v1/consumer/chat-room
    ↓
...
```
프론트엔드 → GET /v1/seller/chat-room/store/:storeId
    ↓
