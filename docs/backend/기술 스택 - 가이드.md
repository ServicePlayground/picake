...
Picake 프로젝트의 백엔드 기술 스택과 데이터베이스 설정에 대한 종합 가이드입니다.
...
DATABASE_URL="postgresql://picake_admin:your_password@localhost:5432/picake_dev_db"
```
...
-- 권한 부여
GRANT ALL PRIVILEGES ON DATABASE picake_dev_db TO picake_admin;
-- 데이터베이스 소유자 변경
ALTER DATABASE picake_dev_db OWNER TO picake_admin;
```
...
# 개발 환경 데이터베이스 리셋
yarn workspace @picake/backend db:reset:dev
...
- 스테이징/프로덕션은 EC2에 직접 설치한 PostgreSQL 사용 ([EC2 배포 가이드(검증)](<../infra/aws/(검증)EC2_Route53_S3_CloudFront.md>) 참고)
...
- **Google / Kakao / Apple OAuth**: 소셜 로그인 (직접 구현, axios 기반. Apple은 `jsonwebtoken`으로 client_secret(JWT) 서명)
- **jwks-rsa**: Apple의 공개키(JWKS)로 `id_token` 서명 검증
- **speakeasy**: Admin TOTP(2단계 인증) 발급/검증
- **bcrypt**: 비밀번호 해싱
- **Helmet**: 보안 헤더 설정
...
- **@nestjs/websockets**: WebSocket 게이트웨이
- **@nestjs/platform-socket.io**: Socket.IO 어댑터
- **socket.io**: 실시간 채팅 및 알림 (채팅 namespace: `/`, 알림 namespace: `/notifications`)

### 알림 (Push Notification / 알림톡)

- **Firebase Cloud Messaging (FCM)**: 모바일 푸시 알림 전송
- **solapi**: 카카오 알림톡 발송

### 에러 모니터링

- **@sentry/nestjs**: Sentry 에러 로깅 및 성능 모니터링
...
- **axios**: HTTP 클라이언트 (Google/Kakao OAuth 등 외부 API 호출)
