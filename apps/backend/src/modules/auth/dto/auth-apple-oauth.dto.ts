...
/**
 * 애플 로그인 요청 DTO (Authorization Code)
 */
export class AppleLoginRequestDto {
  @ApiProperty({
    description: SWAGGER_DESCRIPTIONS.APPLE_CODE,
    example: SWAGGER_EXAMPLES.APPLE_CODE,
  })
  @IsNotEmpty()
  @IsString()
  code: string;
 ...
 * 애플 연동 회원가입 요청 DTO
 */
export class AppleRegisterRequestDto {
  @ApiProperty({
    description: SWAGGER_DESCRIPTIONS.APPLE_ID,
    example: SWAGGER_EXAMPLES.CONSUMER_DATA.appleId,
  })
  @IsNotEmpty()
  @IsString()
  appleId: string;
  ...
  @ApiProperty({
    description: SWAGGER_DESCRIPTIONS.APPLE_EMAIL,
    example: SWAGGER_EXAMPLES.CONSUMER_DATA.appleEmail,
  })
  @IsNotEmpty()
  @IsString()
  appleEmail: string;
  ...
  @ApiProperty({
    description: "개인정보 제3자 제공 동의 여부 (필수)",
    example: true,
  })
  @IsBoolean()
  ...
  @ApiProperty({
    description: "위치기반서비스 이용약관 동의 여부 (선택)",
    example: false,
    required: false,
  })
  @IsOptional()
