...
/**
 * 애플 OAuth 이후 휴대폰 인증·회원가입 전용 화면
 * (콜백에서 appleId·appleEmail 쿼리로 전달)
 */
export function AppleRegisterVerificationScreen() {
  ...
  const handleDuplicateAccount = useCallback((payload: DuplicateAccountPayload) => {
    ...
    trackEvent("view_duplicate_account", { provider: "apple", duplicate_type: "phone_number" });
  }, []);
  const { showAlert } = useAlertStore();
  const appleRegisterMutation = useAppleRegister({ onDuplicateAccount: handleDuplicateAccount });

  const [appleLoginData, setAppleLoginData] = useState<{
    appleId: string;
    appleEmail: string;
  } | null>(null);
  ...
  useEffect(() => {
    const appleId = searchParams.get("appleId")?.trim();
    const appleEmail = searchParams.get("appleEmail")?.trim();
  ...
  const handleSendVerificationCode = async () => {
    trackEvent("request_phone_verification", { provider: "apple" });
    const normalized = normalizePhone(phone);
    const { expiresAt } = await sendPhoneVerificationMutation.mutateAsync({
      phone: normalized,
      purpose: PHONE_VERIFICATION_PURPOSE.APPLE_REGISTRATION,
    });
  ...
  const handleSubmit = async () => {
    ...
    trackEvent("success_phone_verification", { provider: "apple" });
    ...
    trackEvent("request_signup", { provider: "apple" });
  ...
  const registerPending = appleRegisterMutation.isPending;
